...
const ImageMarker = ({ footprint, onClick, isSelected }) => {
  ...
  useEffect(() => {
    const loadImage = async () => {
      ...
      } else if (footprint.photoUrl.includes('undefined%2F')) {
        // URL 인코딩된 undefined/ 제거
        cleanPhotoUrl = footprint.photoUrl.replace('undefined%2F', '');
      ...
      try {
        ...
        const attemptLoad = () => {
          ...
          img.onerror = () => {
            ...
            if (retryCount === 0) {
              console.error('=====================================');
              console.error('🔴 Azure Blob Storage 403 권한 오류');
              console.error('=====================================');
              console.error('백엔드에서 SAS 토큰에 읽기 권한이 없습니다.');
              console.error('해결 방법:');
              console.error('1. Azure Blob Storage SAS 토큰 생성 시 읽기 권한 추가');
              console.error('2. 백엔드 코드에서 BlobSASPermissions.parse("r") 또는 BlobSASPermissions.parse("rw") 설정');
              console.error('3. 현재 설정: sp=cw (Create, Write) → sp=r (Read) 또는 sp=rw (Read, Write)로 변경 필요');
              console.error('=====================================');
            }
      ...
      try {
        ...
        const attemptLoad = () => {
          ...
          img.onerror = () => {
            ...
            if (retryCount === 0) {
              console.error('=====================================');
              console.error('🔴 Azure Blob Storage 403 권한 오류 (상세 이미지)');
              console.error('=====================================');
              console.error('백엔드에서 SAS 토큰에 읽기 권한이 없습니다.');
              console.error('해결 방법:');
              console.error('1. Azure Blob Storage SAS 토큰 생성 시 읽기 권한 추가');
        ...
        console.error('상세 이미지 로드 중 오류:', error);
        setIsLoading(false);
        setImageError(true);
      }
    };
