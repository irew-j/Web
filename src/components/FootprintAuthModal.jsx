  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (photo) {
        try {
          // 1. 파일 이름 생성
          const fileName = `footprint_${new Date().getTime()}_${photo.name}`;
          
          // 2. SAS URL 요청
          const { sasUrl, permanentUrl, error: sasError } = await getSasUrl(fileName);

          if (sasError || !sasUrl) {
            throw new Error(sasError || 'SAS URL을 받아오지 못했습니다.');
          }

          // 3. Azure Blob Storage에 이미지 업로드
          const uploadSuccess = await uploadImageToBlob(sasUrl, photo);
          if (!uploadSuccess) {
          ...
          console.error('이미지 업로드 처리 실패:', uploadError);
          alert(`이미지 업로드에 실패했습니다: ${uploadError.message}`);
          // 이미지 업로드 실패 시 발자국 생성을 중단할 수 있습니다.
          // 또는 photoUrl 없이 생성을 계속 진행할 수 있습니다.
          // 여기서는 일단 중단하지 않고 진행하되, photoUrl은 null로 남습니다.
        }
      }
      ...
      const footprintData = {
        ...
        photoUrl, // 업로드된 이미지의 영구 URL 또는 null
