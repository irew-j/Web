...
// Remove the getToken function as AuthContext will handle global axios headers
// const getToken = () => {
...
export const createFootprint = async (footprintData) => {
  try {
    // Remove explicit token retrieval and header setting
...
export const getImageSasUrl = async (fileName) => {
  try {
    console.log('SAS URL 요청 중:', fileName);
    const response = await axios.post(`${API_URL}/storage/sas-url`, { fileName }, {
      // headers: {
      //   Authorization: `Bearer ${token}`,
      // },
